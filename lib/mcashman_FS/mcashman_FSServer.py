...
def get_config():
    ...
    for nameval in config.items(get_service_name() or 'mcashman_FS'):
...
impl_mcashman_FS = mcashman_FS(config)
...
async_run_methods = {}
async_check_methods = {}
async_run_methods['mcashman_FS.FeatureSelection_async'] = ['mcashman_FS', 'FeatureSelection']
async_check_methods['mcashman_FS.FeatureSelection_check'] = ['mcashman_FS', 'FeatureSelection']
sync_methods['mcashman_FS.FeatureSelection'] = True
...
class Application(object):
    ...
    def __init__(self):
        submod = get_service_name() or 'mcashman_FS'
        ...
        self.rpc_service.add(impl_mcashman_FS.FeatureSelection,
                             name='mcashman_FS.FeatureSelection',
                             types=[dict])
        self.method_authentication['mcashman_FS.FeatureSelection'] = 'required'
                    ...
                    if auth_req != "none":
                        if token is None and auth_req == 'required':
                            ...
                            err.data = "Authentication required for " + \
                                "mcashman_FS but no authentication header was passed"
                            raise err
